...
* **Component groups match wires by filter.** A `"type": "Wire"` component group matches wires by `usageGroups` (values from the wire usage group enum, e.g. `PRIMARY`, `NEUTRAL`, `COMMUNICATION`), by `industry` (`UTILITY`, `COMMUNICATION`, or `BOTH` — compared against the wire owner's industry), and optionally by `includedOwners`/`excludedOwners`, `wireClasses`, or `includedClientItems`. Empty filter lists mean "any". `usageGroups` and `wireClasses` are alternatives on one axis: a wire passes that axis if its usage group is in `usageGroups` **or** its wire class is in `wireClasses`, so groups built purely from usage groups work fine when no wire classes are in play — missing wire classes do not stop a case from running. If you do filter by `wireClasses` (or set `wireClass` on structure wires), those wire classes must be defined in the client file — a wire class that the client file doesn't define triggers a mapping prompt on import (or resolves to none headlessly), so don't reference wire classes unless the utility's client file actually has them. A group that matches no wires in the design generates no rules.
...
* **Weather cases need wire states.** "Vertical to Ground" and "Wire To Wire" cases carry an `upperThermalState` (and optionally, when `checkPhysicalState` is true, an `upperPhysicalState`; Wire To Wire also has `lowerThermalState`/`lowerPhysicalState`). Each is a wire height calculator — usually `"type": "Wire State"` with an inline wire state (tension type plus weather conditions), or `"Line of Sight"`/`"Measured Wire"` to use the measured geometry directly. **Give every inline wire state all three weather conditions** — `designCondition`, `highestStressCondition`, and `creepCondition`, each fully populated including `windSpeed`. The schema's minimum (only `designCondition`, plus `creepCondition` for `FINAL_WITH_CREEP`) is enough to import, but SPIDAcalc's clearance editor reads `highestStressCondition` for any non-`INITIAL` tension type and a missing condition makes the clearance case fail to open (a `Cannot read properties of undefined` error). SPIDAcalc itself always writes all three; do the same.
