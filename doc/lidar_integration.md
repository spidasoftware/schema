...
| Terrain under spans | `wireEndPoints[].terrainPoints` (per span), or project `terrainLayers` + `appliedTerrainLayers` (one lat/lon/elevation ground surface applied to a design layer) | For clearance analysis. Terrain layers additionally require each design `mapLocation` to carry the pole base elevation (meters ASL) as a third GeoJSON coordinate, points are only used within `profileViewWidth` of a span, and wire end point `distance` must be at least the geodesic pole-to-pole distance or far-end points are dropped — see [Terrain Layers](calc.md#terrain-layers) |
...
| Required clearances to check | project `defaultClearanceCases` (defaults for new designs) and design `clearanceCases` (cases actually run on that design) | Ground rows fire per span environment, so lidar land-use classification feeds directly into clearance results — see [Clearance Cases](calc.md#clearance-cases) |
...
Working examples to crib from: [lidar measured project](../resources/examples/spidacalc/projects/lidar_measured_project.json) (start here — a complete "Measured" design built the way this guide describes) · [lidar connected locations project](../resources/examples/spidacalc/projects/lidar_connected_locations_project.json) (three connected poles with connected wires and measured sags on every span) · [lidar end-to-end project](../resources/examples/spidacalc/projects/lidar_end_to_end_project.json) (capstone — a five-pole line combining connectivity, connected wires, measured sags for tension data, terrain points under every span, deadend guying, and equipment) · [lidar terrain layer project](../resources/examples/spidacalc/projects/lidar_terrain_layer_project.json) (five connected poles delivering ground as a project-level terrain layer applied to the Measured Design layer, instead of per-span terrainPoints) · [lidar clearance cases project](../resources/examples/spidacalc/projects/lidar_clearance_cases_project.json) (the terrain layer project extended with Vertical to Ground, Wire To Wire, and At Pole clearance cases applied to every design, with a PEDESTRIAN environment span) · [minimal project with GPS](../resources/examples/spidacalc/projects/minimal_project_with_gps.json) · [full project](../resources/examples/spidacalc/projects/full_project.json) · [one-of-everything structure](../resources/examples/spidacalc/designs/one_of_everything.json)
