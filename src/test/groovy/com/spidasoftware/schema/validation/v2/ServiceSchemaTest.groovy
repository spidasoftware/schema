...
class ServiceSchemaTest extends GroovyTestCase { 
	...
	void testAssetInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidamin/asset/interfaces/asset.json").text)
		instance.each{k,v->
	...
	void testAssetSearchInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidamin/asset/interfaces/asset_search.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testGeoCoderInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidamin/geo/interfaces/geo_coder.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testUsersInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidamin/user/interfaces/users.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testUsersSecurityInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidamin/user/interfaces/users_security.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testWireAnalysisInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidacalc/analysis/interfaces/wire_analysis.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testLoadingAnalysisInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidacalc/analysis/interfaces/loading_analysis.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
	...
	void testClientDataInterfaceMethodsAgainstServiceMethod(){
		def instance = slurper.parseText(new File("v1/spidacalc/client/interfaces/client_data.json").text)
		instance.each{k,v->
			if(k!="id" && k!="description"){
				report = schema.validate(JsonLoader.fromString(JsonOutput.toJson(v)))
				report.each{
