...
class GenericHttpClientSpec extends Specification {
	...
	void "requests parameters should be added to the uri for custom entity"() {
		when: "create the request"
			def request = client.createHttpUriRequest(new Request(httpMethod:"POST", uri:uri, parameters:currentParams, headers:null, customEntity: new StringEntity("test")))
	...
	void "multipart requests should have files added with the correct content dispositions"() {
		setup: "Create a parameter map that includes both string and File values"
			File file = new File(getClass().getResource("/httpClient/test.json").toURI())
	...
	void "multipart requests should have the correct mime type detected"() {
		setup: "Create a parameter map that includes both string and File values"
			def params = [:]
			["rtf", "json", "pdf"].each{
				File file = new File(getClass().getResource("/httpClient/test.${it}").toURI())
		...
		then: "The file's Content-Disposition must include the filename"
			...
			content.contains("application/rtf")
